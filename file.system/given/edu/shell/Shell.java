...
import edu.filesystem.IFileSystem;
...
public class Shell {
  ...
  public void loop() {
    ps.println("Welcome to our shell.");
    ps.println("To get help, type the command: 'help'.");
    ps.println("Enjoy!");
    while (true) {
      ps.print("> ");
      ps.flush();
      try {
        ...
        ps.println("\nInvalid command or internal error:");   
  ...
  private void help(String args[]) {
    if (args == null || args.length == 0) {
      ps.println("Possible commands: pwd, cd, ls, mkdir, rmdir, touch, edit, cat, mv, exit");
      ps.println("To obtain specific help, type:  help <command-name>");
...
//        ps.println(" sync: write all changes to mass storage");
        ...
        ps.println(" Exit this shell.");
        ps.println(" Usage: exit");
      } else if (cmd.equals("cd")) {
        ps.println(" Change current directory");
        ps.println(" Usage:  cd <path>");
      } else if (cmd.equals("ls")) {
        ps.println(" List the contents of a directory, given as a path");
        ps.println(" Usage:  ls <path>");
      } else if (cmd.equals("mkdir")) {
        ps.println(" Makes a directory, given as an absolute or relative path");
        ps.println(" Usage:  mkdir <path>");
      } else if (cmd.equals("rmdir")) {
        ps.println(" Removes a directory, given as an absolute or relative path");
        ps.println(" Usage:  rmdir <path>");
      } else if (cmd.equals("touch")) {
        ps.println(" Creates an empty file, given as an absolute or relative path");
        ps.println(" Usage:  touch <path>");
      } else if (cmd.equals("rm")) {
        ps.println(" Removes a file, given as an absolute or relative path");
        ps.println(" Usage:  rm <path>");
      } else if (cmd.equals("edit")) {
        ps.println(" Allows to append lines of text to a file");
        ps.println(" Usage:  edit <path>");
      } else if (cmd.equals("cat")) {
        ps.println(" Prints the contents of a file, given as an absolute or relative path");
        ps.println(" Usage:  cat <path>");
      } else if (cmd.equals("mv")) {
        ps.println(" Moves a directory or a file, given as an absolute or relative path ");
        ps.println(" to the current directory, with the given name in ");
        ps.println(" Usage: mv <path> <name>");
      }
    }
  }
...
//      fs.sync();
   ...
   */
  private IDirectory nameToDir(IDirectory d, String name, boolean create) {
    name = name.trim();
    if (name.length() == 0)
      return d;
    if (name.equals("."))
   ...
   * @throw FSException if the path is not a valid path to a directory.
   */
  private IDirectory pathToDir(String path, boolean create) {
    IDirectory d = wd;
    if (path.startsWith("/")) {
      d = root;
   ...
   */
  private IFile pathToFile(String path, boolean create) {
    IDirectory d = wd;
    if (path.startsWith("/")) {
      d = root;
      path = path.substring(1);
  ...
  private void edit(String args[]) {
    ...
    if (file != null) {
      ...
      try {
        ...
        // consider that an end of file situation.
      }
    } else
      fail("Not found:" + path);
  }
  ...
  /*
   * Moves a resource, given as a path (relative or absolute), 
   * to the current directory, with the given name.
   *  
   *  mv path name
   *  
  ...
  private void mv(String args[]) {
    ...
    String name = args[1];
    if (name.indexOf('/') != -1)
      throw new IllegalArgumentException("Given name must not be a path (cannot use '/').");
